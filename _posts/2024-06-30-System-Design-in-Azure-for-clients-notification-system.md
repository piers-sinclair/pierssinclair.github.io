...
A: They should be notified if someone comments on a post that they have made, subscribed to or commented on.
...
- Set a user's subscription on a post when they post, comment or subscribe.
...
Q: In that case, I recommend we have a user settings page to give user's control so they don't feel intruded upon. Do you agree?\
A: Yep sounds like a good idea.

Notice how the last question brought into light a new functional requirement? These can be discovered at anytime so make sure you add it to your list:
...
The first problem to tackle, is how do we adapt the system to track who should get notifications and when?
...
The first item is a `notification service`, when the API triggers a notification event (e.g. a comment) it will send a request to the `notification service`. This service then reads from the users database to get the list of users to send notifications to.
...
One way to decouple is to follow the [Publisher-Subscriber pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/publisher-subscriber) through a messaging system. This pattern will ensure that the `notification service` is fire and forget. It simply puts a message on the queue and let's the `notification channel services` figure out how to process it.
...
Our notifications aren't mission critical and there is potential for the user to be sent lots of notifications in a small time period. For this reason, we should [rate limit](https://learn.microsoft.com/en-us/azure/architecture/patterns/rate-limiting-pattern) the notifications to ensure a better UX.

In our case, we can set a limit of 1 notification per post per hour. That way the user gets notifications of different posts, but won't be spammed by multiple comments on the same post. We can do rate limiting easily inside of the code of our services.

### 4. How do we ensure resiliency and report on faults in 3rd party services?

Utilising 3rd party services like SendGrid is awesome for reducing the code we need to write, and ensuring a highly resilient system. However, it can be a pain if the system goes down.

There's a few important factors to consider:
- A service might go down for a short period of time before coming back up
- The devs will need to be alerted and provided information to debug an outage when it occurs.
...
For logging and alerting, [Application Insights](https://learn.microsoft.com/en-us/azure/azure-monitor/app/app-insights-overview) is a powerful tool we can enable on our [Azure Function services](https://learn.microsoft.com/en-us/azure/azure-functions/configure-monitoring?tabs=v2). Application Insights gives us a lot of what we need out-of-the-box and anything else needed can be done with manual calls to the API.

Retry is straight forward, we want to follow the [retry pattern](https://learn.microsoft.com/en-us/azure/architecture/patterns/retry). At a basic level this involves:
...
- Fault Tolerant - In the event of message queue processing failures, the system makes use of retries and dead-letter queues.
- Maintainability - We have Application Insights in place to alert us of errors and log issues.


#### Deficiencies
- Complexity - The system is highly complex with lots of moving parts and will also make deployment more difficult.
- Availability - Our system hasn't factored in whether a country has access to the 3rd party services we are using. This is not a huge deal because most customers are in the US.
- Not realtime - Our notifications won't necessarily be delivered quickly. This would be a problem if we needed to send a notification for something like login or a security issue.
